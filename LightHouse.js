...
async function captureReport() {
	...
	//================================SELECTORS================================
	// const tableTab = "<a href='/category/tables'>Tables</a>";
	...
	const successMessage = "div.react-toast-notifications__container.css-1oqa81j";
	const cartIcon = "button.icon-cart";
	const checkout   = "a.default-btn[href='/checkout']";
	...
	await flow.startTimespan({ stepName: 'add table to cart' });
		...
		await page.waitForSelector(successMessage);
	await flow.endTimespan();
	console.log('table is added to cart (via success message)');

	await flow.startTimespan({ stepName: 'click cart icon' });
		await page.click(cartIcon);
		await waitTillHTMLRendered(page);
		await page.waitForSelector(checkout);
	await flow.endTimespan();
	...
	await flow.startTimespan({ stepName: 'proceed to checkout' });
		...
		//await page.waitForSelector(checkout);
	await flow.endTimespan();
	console.log('user is redirected to the checkout page');
